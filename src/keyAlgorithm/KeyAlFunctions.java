...
public class KeyAlFunctions {

    // =================================================================
    /**
     * In ra giá trị của từ Lưu ý: 1 từ 32 bit sẽ có 4 bytes, tương đương 8 chữ
     * số hex
     *
     * @param 
     */
...
//          System.out.println(w);
        for (int i = 1; i <= 8; i++) {
            int hexan = (w >> 32 - i * 4) & 0xF; // Di chuyển 4 bit và giữ lại 1 kí tự hex
            System.out.printf(Integer.toHexString(hexan));
        }
        System.out.printf("\n");
    }
    ...
    public static int RotWord(int w) {
        ...
        int rot = (byte234 << 8) | byte1; // 3 byte cuối được di chuyển 8 bit (1byte) và nối với byte1
        // Phép toán này hoạt động bằng cách so sánh từng cặp bit tương ứng giữa hai số và áp dụng quy tắc sau:
        // nếu ít nhất một trong hai bit là 1, thì bit kết quả sẽ là 1; 
        // nếu cả hai bit đều là 0, thì bit kết quả cũng sẽ là 0.  
        ShowWord(rot);
        return rot;
    }
...
//        System.out.println(Integer.toHexString(w));
        // Rcon[9] = 0x1B: Tại điểm này, do 0x80 được nhân đôi, chúng ta vượt qua 0xFF và do đó, XOR với 0x11B giúp trả về vào trường GF(2^8).
...
//        ShowWord(kq);
    ...
    public static int[] AddRoundKey(int[] state, int[] key) {
        ...
        kq[0] = state[0] ^ key[0]; // Thực hiện XOR giữa các vị trí 0 của hai mảng
        kq[1] = state[1] ^ key[1]; // Thực hiện XOR giữa các vị trí 1 của hai mảng
        kq[2] = state[2] ^ key[2]; // Thực hiện XOR giữa các vị trí 2 của hai mảng
        kq[3] = state[3] ^ key[3]; // Thực hiện XOR giữa các vị trí 3 của hai mảng 
//        for(int i=0; i<4; i++)
//            ShowWord(kq[i]);
    ...
    // Duyệt hàng
        for (int i = 0; i < 4; i++) {
//            System.out.println("Stateeee    " + Integer.toHexString(state[i]));
...
//            System.out.println(Integer.toHexString(byte1));
    ...
    public static int MultiplyTwo(int w) { 
        int kq = w << 1; 
        if (kq > 255) { // Nếu tích lớn hơn 256
...
//                03  01  01  02 ]
        ...
        int kq;
        int byte1 = (w >> 24) & 0xFF; // Lấy 8 bit cuối của byte 1  0x000000d4
    ...
    public static int[] InvShiftRows(int[] state) {
        ...
        for (int i = 0; i < 4; i++) {
           ...
           // System.out.println("Hex  " + Integer.toHexString(byte1) + "  " + Integer.toHexString(byte2) + "  " + Integer.toHexString(byte3) + "  " + Integer.toHexString(byte4));
